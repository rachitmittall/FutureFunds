      ...
      try {
        const entries = await Promise.all(
          STOCKS.map(async ([name, ticker]) => {
            ...
            const prevClose = Number(meta?.chartPreviousClose ?? regular)
            const changePct = prevClose ? ((regular - prevClose) / prevClose) * 100 : 0
            return [ticker, { name, ticker, price: regular, changePct }]
          }),
        )
        if (!mounted) return
...
export default function Investments({
  ...
  const { prices: stockPrices, loading: stocksLoading, error: stocksError } = useStocksData()

  const hdfc = usePollingJson('https://api.mfapi.in/mf/118989', { fallback: null })
  const mirae = usePollingJson('https://api.mfapi.in/mf/118778', { fallback: null })
  const gold = usePollingJson('https://api.metals.live/v1/spot/gold', { fallback: null })
  const silver = usePollingJson('https://api.metals.live/v1/spot/silver', { fallback: null })
  const crypto = usePollingJson(
  ...
  const stockTotal = useMemo(() => {
    return Object.values(stockPortfolio || {}).reduce((sum, row) => {
      const shares = Number(row?.shares ?? 0)
      const price = Number(stockPrices[row?.ticker]?.price ?? row?.price ?? 0)
      return sum + Math.max(0, shares) * Math.max(0, price)
    }, 0)
  ...
  const goldInrPerGram = (() => {
    const usdPerOz = Number(gold.data?.[0]?.[1])
    if (!Number.isFinite(usdPerOz)) return null
    return usdPerOz * 0.0321507 * 83.5
  })()
  const silverInrPerGram = (() => {
    const usdPerOz = Number(silver.data?.[0]?.[1])
    if (!Number.isFinite(usdPerOz)) return null
    return usdPerOz * 0.0321507 * 83.5
  })()
  const mfRows = [
    { name: 'HDFC Top 100 Fund', nav: Number(hdfc.data?.data?.[0]?.nav ?? 0) || null },
    { name: 'Mirae Asset Large Cap', nav: Number(mirae.data?.data?.[0]?.nav ?? 0) || null },
  ]
    ...
    <div className="pb-24 md:pb-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="rounded-2xl bg-ff-card border border-ff-border px-4 py-3 shadow-lg">
          ...
          <div className="mt-1 text-lg font-extrabold text-white">
            <span className="text-ff-neon"><AnimatedNumber value={totalAllocated} format={formatINR} /></span> / {formatINR(available)}
          </div>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border-l-4 border-ff-neon bg-ff-card p-5 shadow-lg">
          <div className="text-lg font-extrabold text-white">📈 Stocks (NSE)</div>
          <input
            value={search}
          ...
          {stocksError ? <div className="mt-2 text-xs text-ff-danger/80">Stock prices unavailable, showing cached values.</div> : null}
          ...
          <div className="mt-4 rounded-xl bg-ff-bg border border-ff-border p-3">
            ...
            <div className="mt-3 text-sm font-extrabold text-ff-textSec pt-2 border-t border-ff-border">
              Total stocks investment: <span className="text-ff-neon"><AnimatedNumber value={stockTotal} format={formatINR} /></span>
            </div>
          </div>
        </div>

        <div className="rounded-2xl border-l-4 border-ff-blue bg-ff-card p-5 shadow-lg">
          <div className="text-lg font-extrabold text-white">🏦 Mutual Funds <span className="ml-2 rounded-full border border-ff-blue/30 bg-ff-blue/10 px-2 py-0.5 text-[10px] text-ff-blue shadow-glow-blue">LIVE</span></div>
        ...
        <div className="rounded-2xl border-l-4 border-ff-gold bg-ff-card p-5 shadow-lg">
          <div className="text-lg font-extrabold text-white">🥇 Gold</div>
        ...
        <div className="rounded-2xl border-l-4 border-ff-silver bg-ff-card p-5 shadow-lg">
          <div className="text-lg font-extrabold text-white">🥈 Silver</div>
        ...
        <div className="rounded-2xl border-l-4 border-ff-danger bg-ff-card p-5 shadow-lg">
          <div className="text-lg font-extrabold text-white">₿ Crypto <span className="ml-2 rounded-full border border-ff-danger/30 bg-ff-danger/10 px-2 py-0.5 text-[10px] text-ff-danger shadow-glow-red">LIVE</span></div>
          <div className="mt-3 space-y-2 text-sm">
